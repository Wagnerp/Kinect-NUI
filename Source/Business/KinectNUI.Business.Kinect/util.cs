...
namespace KinectNUI.Business.Kinect
{
	public static class util
	{
		...
		public static byte[] convertDepthFrame2(byte[] depthFrame16, ref byte[] depthFrame32)
		{
			int GREEN_IDX = 1, RED_IDX = 2, BLUE_IDX = 0;
			...
			for (int i16 = 0, i32 = 0; i16 < depthFrame16.Length && i32 < depthFrame32.Length; i16 += 2, i32 += 4)
			{
				...
				int depth = (depthFrame16[i16 + 1] << 5) | (depthFrame16[i16] >> 3);
		...
		public static Polyline getBodySegment(JointCollection joints, Brush brush, ref KinectSensor nui, ref Canvas skeleton, params JointType[] ids)
		{
        ...
        private static Point getDisplayPosition(Joint joint, ref KinectSensor nui, ref Canvas skeleton)
        {
		...
		public static Point getDisplayPosition(Joint joint, ref KinectSensor nui, ref Canvas skeleton, DepthImageFrame depthFrame)
		{
			float depthX, depthY;
            DepthImagePoint depthPoint = nui.MapSkeletonPointToDepth(joint.Position, depthImageFormat);
            depthX = depthPoint.X;
            depthY = depthPoint.Y;
			depthX = Math.Max(0, Math.Min(depthX * 320, 320));  //convert to 320, 240 space
			depthY = Math.Max(0, Math.Min(depthY * 240, 240));  //convert to 320, 240 space
            int colorX, colorY;
            ColorImagePoint colorPoint = depthFrame.MapToColorImagePoint(depthPoint.X, depthPoint.Y, nui.ColorStream.Format);
            colorX = colorPoint.X;
            colorY = colorPoint.Y;
        ...
        //public static Point getDisplayPosition(Joint joint, ref KinectSensor nui, ref Canvas skeleton)
        //{
        //    throw new NotImplementedException();
        //}
    }
